...
if ( ! class_exists('WoobizzFooterOptions') ) {
	class WoobizzFooterOptions{
		...
		public function __construct( 
			...
			$this->_prefix	= $prefix;
			$this->_url		= ( $url ) ? $url : plugins_url().'/woobizz-footer';
			$this->_folder	= $folder;
		...
		function create_page() {
			if ( $this->_options ) {
				...
				if ( isset( $tabs ) ) {
					...
					foreach ( $tabs as $tab ) {
						...
						if ( isset( ${'tab_'.$tab['id']} ) ) {
							foreach ( ${'tab_'.$tab['id']} as $tab_content ) {
								$option = new WoobizzFooterOptionsGenerator( $tab_content, $this->_prefix );
								$div_contents .= $option;
							}
						} else {
					...
					foreach ( ${'tab_0'} as $tab_content ) {
						$option = new WoobizzFooterOptionsGenerator( $tab_content );
						$div_contents .= $option;
					}
			...
			$html .= '<div id="options-saved"><div></div><strong>'.__('Saved','whitelabel').'</strong></div>';
			$html .= '<div id="options-error"><div></div><strong>'.__('Error','whitelabel').'</strong></div>';
...
if ( ! class_exists( 'WoobizzFooterOptionsGenerator' ) ) {
	class WoobizzFooterOptionsGenerator {
